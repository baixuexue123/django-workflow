...
class ViewTestCase(TestCase):
        ...
        def test_dotfile(self):
            ...
            self.assertContains(response, 'A definition for a diagram of the workflow: test workflow')
        ...
        def test_graphviz_with_no_graphviz(self):
            ...
            try:
                ...
                self.assertEqual(
                    u"GRAPHVIZ_DOT_COMMAND constant not set in "
                    u"settings.py (to specify the absolute path to graphviz's dot command)",
                    instance.args[0]
                )
            else:
